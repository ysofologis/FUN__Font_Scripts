...
def scale_font_glyphs(input_path: str, output_path: str, scale_percent: float, thickness_percent: float = 0) -> bool:
    ...
    --thickness value (v8.4 RADIAL DILATION):
      Thickens stems by applying radial dilation to each contour from the
      glyph center. Detects outer vs inner contour by signed area (shoelace
      formula):
        - OUTER contour: each point pushed OUTWARD (away from glyph center)
          by dx = thickness_percent * W / 100 units.
          → outer contour expands, glyph grows slightly
        - INNER counter: each point pushed INWARD (toward glyph center)
          by the same amount.
          → counter shrinks, stems look thicker
      Effect: stems thicker on BOTH sides, glyph slightly bigger, advance
      widths unchanged. This is similar to FontForge's changeWeight.

      v8.3 (DEPRECATED): used inset-rescale on every contour, which shrunk
      the outer contour too (making the glyph appear smaller).
      v8.2 (DEPRECATED): non-uniform X scale that grew the whole font.
...
def _thicken_truetype_glyphs(font: TTFont, thickness_percent: float) -> None:
    """Apply v8.4 RADIAL DILATION for true stem-thickening.

    Algorithm (v8.4 IMPROVED over v8.3):
      1. For each glyph, compute the GLYPH CENTER (cx, cy).
      2. Compute dx = thickness_percent * W / 100 (radial dilate amount).
      3. For each contour, compute SIGNED AREA (shoelace formula) to identify
         outer vs inner:
           - TrueType: negative signed area = OUTER, positive = INNER counter
      4. For each point on the contour:
           - OUTER contour: push point OUTWARD (away from glyph center) by dx
             → glyph grows slightly, outer contour expands
           - INNER counter: push point INWARD (toward glyph center) by dx
             → counter shrinks, stems look thicker

    Effect:
      - Stems appear thicker on BOTH sides (outer grows, inner shrinks).
      - Glyph grows slightly (by approximately `thickness% of width`).
      - Advance width unchanged.
      - This is similar to FontForge's changeWeight algorithm.
    """
    glyf = font['glyf']
    ...
    for glyph_name in font.getGlyphOrder():
        ...
        if glyph.numberOfContours == 0:
            ...
            # Glyph center for radial direction
            gx_center = (xmin_g + xmax_g) / 2.0
            gy_center = (ymin_g + ymax_g) / 2.0
            ...
            for start_idx, end_idx in ranges:
                ...
                # Signed area to detect outer vs inner
                n = len(pts)
                area = 0.0
                for j in range(n):
                    x1, y1 = pts[j]
                    ...
                    area += x1 * y2 - x2 * y1
                area /= 2.0
                is_outer = area < 0  # TrueType convention
                # For single-contour glyphs (no holes), all is outer
                if len(ranges) == 1:
                    is_outer = True

                for j in range(start_idx, end_idx):
                    x, y = old_coords[j]
                    # Direction from glyph center to point
                    vx, vy = x - gx_center, y - gy_center
                    d = (vx * vx + vy * vy) ** 0.5
                    if d < 0.001:
                        # Point at glyph center: leave unchanged
                        new_coords_list[j] = (int(x), int(y))
                        continue
                    ux, uy = vx / d, vy / d
                    if is_outer:
                        # Push OUTWARD (away from center)
                        nx = int(round(x + ux * dx))
                        ny = int(round(y + uy * dy))
                    else:
            ...
            # Use GlyphCoordinates so fontTools internals work
...
def _thicken_cff_glyphs(font: TTFont, thickness_percent: float) -> None:
    """Apply v8.4 RADIAL DILATION to CFF charstrings for true stem-thickening.

    Algorithm:
      1. For each glyph, compute glyph center (gx, gy).
      2. For each contour, compute SIGNED AREA to identify outer vs inner:
         - CFF winding: OUTER contour has POSITIVE signed area
         - INNER counter has NEGATIVE signed area
      3. For each point on the contour:
         - OUTER: push OUTWARD (away from glyph center) by dx/dy
         - INNER: push INWARD (toward glyph center) by dx/dy

    Effect: stems thicker on BOTH sides, glyph slightly bigger.
    """
    cff_table_key = 'CFF2' if 'CFF2' in font else 'CFF '
    top_dict = font[cff_table_key].cff.topDictIndex[0]
    char_strings = top_dict.CharStrings
    ...
    class _PerContourRadialPen:
        """Pen that applies RADIAL DILATION using per-subpath direction.

        For each subpath, the is_outer flag (from signed area) determines
        whether we push points outward or inward. The dx/dy for radial
        dilation is applied in the direction (point - glyph_center).
        """
        def __init__(self, outPen, glyph_center, dx, dy, contour_is_outer):
            self._outPen = outPen
            self.gx_c, self.gy_c = glyph_center
            self.dx_global = dx
            self.dy_global = dy
            self._contour_is_outer = iter(contour_is_outer)
            self._is_outer = True  # default

        def _update(self):
            try:
                self._is_outer = next(self._contour_is_outer)
            except StopIteration:
                pass

        def _dialate(self, pt):
            vx, vy = pt[0] - self.gx_c, pt[1] - self.gy_c
            d = (vx * vx + vy * vy) ** 0.5
            if d < 0.001:
                return pt
            ux, uy = vx / d, vy / d
            sign = 1.0 if self._is_outer else -1.0
            return (pt[0] + sign * ux * self.dx_global,
                    pt[1] + sign * uy * self.dy_global)

        def moveTo(self, pt):
            self._update()
            self._outPen.moveTo(self._dialate(pt))

        def lineTo(self, pt):
            self._outPen.lineTo(self._dialate(pt))

        def curveTo(self, *pts):
            self._outPen.curveTo(*(self._dialate(p) for p in pts))

        def qCurveTo(self, *pts):
            self._outPen.qCurveTo(*(self._dialate(p) for p in pts))
    ...
    for glyph_name in font.getGlyphOrder():
        ...
        try:
            ...
            # Compute glyph bbox (for dx/dy + glyph center)
            ...
            dx = max(0.0, thickness_percent * W / 100.0)
            dy = max(0.0, thickness_percent * H / 200.0)
            gx_c = (xmin_g + xmax_g) / 2.0
            gy_c = (ymin_g + ymax_g) / 2.0

            # Per-subpath: compute signed area to determine outer/inner
            # For CFF: positive area = OUTER, negative area = INNER counter
            from fontTools.pens.recordingPen import RecordingPen

            class _SubpathAreaPen(RecordingPen):
                def __init__(self):
                    super().__init__()
                    self._sub_xs = []
                    self._sub_ys = []
                    self.sub_signed_area = []
                def moveTo(self, pt):
                    self._flush()
                    super().moveTo(pt)
                    self._sub_xs = [pt[0]]
                ...
                def _flush(self):
                    if self._sub_xs:
                        ...
                        for j in range(n):
                            ...
                            area += x1 * y2 - x2 * y1
                        self.sub_signed_area.append(area / 2.0)
                        self._sub_xs = []
                        self._sub_ys = []

            rec = _SubpathAreaPen()
            glyph_obj.draw(rec)
            # For CFF: positive area = OUTER
            contour_is_outer = [a > 0 for a in rec.sub_signed_area]
            # If only one subpath, it's the outer
            if len(contour_is_outer) == 1:
                contour_is_outer = [True]

            # Apply radial dilation
            ...
            rad_pen = _PerContourRadialPen(t2_pen, (gx_c, gy_c), dx, dy, contour_is_outer)
            glyph_obj.draw(rad_pen)
...
v8.2 NEW — Hint tuning for Samsung-style fonts without BlueValues:
  ...
  %(prog)s --solid --hint-tune --shape-cleanup --gasp-detail aggressive input_fonts/ output_fonts/
    ...
    parser.add_argument(
        ...
        help="[v8.4 RADIAL DILATION] Thicken stems by dilating each contour "
             "from the glyph center. Outer contour expands outward, inner "
             "counter shrinks inward → stems appear thicker on both sides. "
             "Glyph grows slightly (~thickness%% of width), but advance "
             "widths are preserved. Similar to FontForge's changeWeight. "
             "e.g. 2.5 = subtle bolder stems, 10 = clearly thicker. "
             "Recommended: 1-10. Works independently of --scale."
    )
