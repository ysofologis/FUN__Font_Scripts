#!/usr/bin/env python3
"""
OTF/TTF Font Optimization Script v8.5
...
def _resolve_width_or_die(args) -> None:
    """v8.5 NEW: Validate --expand/--condense/--width combination.

    --width and --expand are mutually exclusive.
    --width and --condense are mutually exclusive.
    """
    explicit_width = args.width_percent != 0
    if args.expand_percent is not None and explicit_width:
        print("ERROR: --width and --expand are mutually exclusive. "
              "Use --width N (positive) or --expand N.", file=sys.stderr)
        sys.exit(2)
    if args.condense_percent is not None and explicit_width:
        print("ERROR: --width and --condense are mutually exclusive. "
              "Use --width -N (negative) or --condense N.", file=sys.stderr)
        sys.exit(2)
    if args.expand_percent is not None and args.condense_percent is not None:
        print("ERROR: --expand and --condense are mutually exclusive. "
              "Use --width N for the exact value.", file=sys.stderr)
        sys.exit(2)


def _resolve_width(args) -> float:
    """v8.5 NEW: Compute final width_percent from --width/--expand/--condense.

    --expand N  -> positive width (expand)
    --condense N -> negative width (condense)
    --width N -> used as-is
    """
...
# ---------------------------------------------------------------------------
#  v8.5 NEW: Horizontal width adjust (expand / condense)
# ---------------------------------------------------------------------------

def width_font_glyphs(input_path: str, output_path: str, width_percent: float) -> bool:
    """
    Adjust horizontal width only (v8.5 NEW).

    Independent of --scale and --thickness. Affects:
      - Glyph outlines (horizontal-only non-uniform scale)
      - Advance widths (preserved spacing)
    ...
    Use cases:
      - Create semi-condensed variants: --width -8
      - Create expanded variants for headers: --width 5
      - Compensate for a font that's too narrow: --width 3
    """
    if width_percent == 0:
        return False

    # Safety caps (same as --scale)
    ...
    logger.info(f"Width adjusting ({direction} by {abs(width_percent):.2f}%%, "
                f"x{factor:.4f})...")
    ...
    try:
        ...
        font.save(output_path)
        font.close()
        logger.info(f"Width-adjusted font saved to {output_path}")
        return True
...
def optimize_font(input_path: str, output_path: str, options: dict) -> bool:
    ...
    # v8.5 NEW: Step 1b — Width adjust (expand/condense horizontally)
    # ------------------------------------------------------------------
    if width_pct != 0:
        ext = os.path.splitext(actual_input)[1] or '.otf'
        fd, temp_path = tempfile.mkstemp(suffix=ext)
        os.close(fd)
        temp_files.append(temp_path)
        font_name = os.path.basename(actual_input)
        logger.info(f"Width-adjusting {font_name} by {width_pct:+.2f}%...")
        if not width_font_glyphs(actual_input, temp_path, width_pct):
            logger.error(f"Width adjustment failed for {font_name}")
            for t in temp_files:
                if os.path.exists(t):
                    os.unlink(t)
            return False
...
v8.2 NEW — Hint tuning for Samsung-style fonts without BlueValues:
  ...
  %(prog)s --solid --hint-tune --shape-cleanup --gasp-detail aggressive input_fonts/ output_fonts/
    ...
    # ---- v8.5 NEW: Horizontal width adjust (expand / condense) ----
    parser.add_argument(
        "--width", type=float, default=0, dest="width_percent",
        help="[v8.5 NEW] Adjust horizontal width. Positive = expand "
             "(e.g. 5 = 5%% wider), negative = condense (e.g. -8 = 8%% narrower). "
             "Affects outlines + advance widths + horizontal metrics. "
             "Independent of --scale (which is uniform). Default: 0."
    )
    parser.add_argument(
        "--expand", type=float, default=None, dest="expand_percent",
        help="[v8.5 NEW] Alias for --width with positive value. "
             "e.g. --expand 5 = --width 5. Mutually exclusive with --width."
    )
    parser.add_argument(
        "--condense", type=float, default=None, dest="condense_percent",
        help="[v8.5 NEW] Alias for --width with negative value. "
             "e.g. --condense 8 = --width -8. Mutually exclusive with --width."
    )
    parser.add_argument(
        "--no-thickness", action="store_true", dest="no_thickness",
        help="[v8.5 NEW] Disable --thickness (escape hatch for scripts "
             "that pass --thickness 0 from elsewhere)."
    )

    # ---- v8.5 NEW: BlueValues quantisation ----
    parser.add_argument(
        "--blue-quantise", type=int, default=0, dest="blue_quantise",
        help="[v8.5 NEW] Round CFF BlueValues/OtherBlues/FamilyBlues "
             "to a clean N-unit grid. Eliminates fractional values that "
             "can cause rasteriser mis-snap. 0 = no quantisation (default), "
             "1 = each unit (highest precision), 2/4/8 = cleaner grid."
    )
    ...
    # v8.5 NEW: Resolve --expand/--condense aliases into --width with validation
    ...
    options = {
        ...
        # v8.5 NEW: Width adjust (with --expand/--condense alias resolution)
