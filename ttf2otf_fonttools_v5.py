#!/usr/bin/env python3
"""
ttf2otf_fonttools_v5.py — TTF → OTF (CFF) converter using fonttools.
...
Improvements over v4:
  - removeHinting() on every source glyph before conversion
  - GASP table set to 0x0F at size 0 → forces AA rendering at all sizes
  - Tighter default max_err (0.3) → sharper curves
  - Glyph.compact() after rebuild → cleaner CFF outlines
  - Head flags updated to indicate no hinting

Usage:
    python ttf2otf_fonttools_v5.py <input_dir> <output_dir>
    python ttf2otf_fonttools_v5.py <input_dir> <output_dir> --recursive
    python ttf2otf_fonttools_v5.py <input_dir> <output_dir> --max-err 0.2
...
def _build_gasp_table(otf: TTFont) -> None:
    """
    Add a GASP table that forces anti-aliasing at all sizes.

    GASP_DOGRAY (0x02) = grayscale antialiasing
    GASP_GRIDFIT (0x01) = grid-fitting (not needed for CFF outlines)
    0x0F = all flags set (future-proof)

    rangeMaxPPEM=0 means "for all sizes".
    """
    gasp = table__g_a_s_p()
...
def convert_ttf_to_otf(ttf_path: Path, otf_path: Path, max_err: float = 0.3) -> dict:
    """
    ...
    # --- Strip TrueType hinting from every glyph before conversion ---
    if "glyf" in ttf:
        for gname in glyph_order:
            if gname in ttf["glyf"]:
                ttf["glyf"][gname].removeHinting()
        # Also remove TrueType instruction tables
        for tbl in ("fpgm", "prep", "cvt "):
            if tbl in ttf:
                del ttf[tbl]
    ...
    # Head table — clear hinting-related flags for CFF fonts
    # Bit 0 (1)  = baseline for font at y=0
    # Bit 1 (2)  = LSB in MS requires bit 0 set
    # Bit 3 (8)  = instructions may depend on point size
    # Bit 4 (16) = force integer ppem
    # Bit 5 (32) = instructions may not alter advance width
    # For hintless CFF: clear bits 3, 4 → 0b0000_0011 = 3
    head_flags = head.flags & ~0x38  # clear bits 3,4,5
    fb.setupHead(
    ...
    # --- GASP table: force AA at all sizes ---
    _build_gasp_table(fb.font)
...
def process_directory(input_dir: Path, output_dir: Path, recursive: bool = False,
    ...
    print(f"Max quadratic→cubic error: {max_err} font units (lower = sharper)")
    print(f"Features: removeHinting + GASP 0x0F + compact()\n")
...
def main():
    ...
    parser.add_argument("--max-err", type=float, default=0.3,
                        help="Max quadratic→cubic error in font units (default: 0.3). "
